...
<div class="ptp-dashboard" id="trainerDashboard">
    ...
    <div class="ptp-dashboard-main">
        ...
        <div class="ptp-tabs">
            ...
            <button class="ptp-tab" data-tab="referrals">Camp Referrals</button>
        ...
        <!-- Referrals Tab -->
        <div class="ptp-tab-content" data-content="referrals">
            <div class="ptp-dashboard-grid">
                <!-- Referral Stats -->
                <div class="ptp-dashboard-card">
                    <div class="ptp-dashboard-card-header">
                        <h2>Your Referral Performance</h2>
                    </div>
                    
                    <div class="ptp-referral-stats" id="referralStats">
                        <div class="ptp-referral-stat">
                            <span class="ptp-referral-stat-value" id="refTotalClicks">-</span>
                            <span class="ptp-referral-stat-label">Link Clicks</span>
                        </div>
                        <div class="ptp-referral-stat">
                            <span class="ptp-referral-stat-value" id="refTotalConversions">-</span>
                            <span class="ptp-referral-stat-label">Registrations</span>
                        </div>
                        <div class="ptp-referral-stat">
                            <span class="ptp-referral-stat-value" id="refConversionRate">-</span>
                            <span class="ptp-referral-stat-label">Conversion Rate</span>
                        </div>
                        <div class="ptp-referral-stat highlight">
                            <span class="ptp-referral-stat-value" id="refTotalCommission">-</span>
                            <span class="ptp-referral-stat-label">Total Earned</span>
                        </div>
                    </div>
                    
                    <p style="margin-top: 16px; color: var(--ptp-gray-500); font-size: 14px;">
                        Earn <?php echo esc_html($referral_commission); ?>% commission when families register for PTP camps and clinics using your referral link!
                    </p>
                </div>
                ...
                <div class="ptp-dashboard-card">
                    ...
                    <div class="ptp-referral-generate">
                        <p>Get your unique referral link to share with families:</p>
                        
                        <div class="ptp-form-group">
                            <label>Link Type</label>
                            <select id="referralType">
                                <option value="general">General (All Camps & Clinics)</option>
                                <option value="summer">Summer Camps 2026</option>
                                <option value="clinics">Training Clinics</option>
                            </select>
                        </div>
                        
                        <button type="button" class="ptp-btn ptp-btn-primary" id="generateReferralLink">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                            Generate Link
                        </button>
                        
                        <div class="ptp-referral-link-box" id="referralLinkBox" style="display: none;">
                            <label>Your Referral Link</label>
                            <div class="ptp-referral-link-input">
                                <input type="text" id="referralLinkUrl" readonly />
                                <button type="button" class="ptp-btn ptp-btn-outline" id="copyReferralLink">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                                    Copy
                                </button>
                            </div>
                            <p class="ptp-referral-code">Your code: <strong id="referralCode"></strong></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="ptp-dashboard-card" style="margin-top: 24px;">
                <div class="ptp-dashboard-card-header">
                    <h2>Your Active Referral Links</h2>
                </div>
                
                <div id="activeReferrals">
                    <p style="color: var(--ptp-gray-500); font-size: 14px;">Loading your referral links...</p>
                </div>
            </div>
            
            <!-- Camp Referral Promo -->
            <div class="ptp-referral-promo" style="margin-top: 24px; background: linear-gradient(135deg, #FCB900 0%, #F59E0B 100%); border-radius: 12px; padding: 24px; color: #0E0F11;">
                <h3 style="margin: 0 0 8px; font-size: 20px;">💰 Boost Your Earnings!</h3>
                <p style="margin: 0 0 16px; opacity: 0.9;">Share your referral link with the families you train. When they sign up for PTP summer camps or clinics, you earn <?php echo $referral_commission; ?>% of their registration!</p>
                <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                    <a href="<?php echo home_url('/camps/'); ?>" class="ptp-btn ptp-btn-secondary" target="_blank">View All Camps</a>
                    <a href="<?php echo home_url('/clinics/'); ?>" class="ptp-btn ptp-btn-outline" target="_blank" style="border-color: #0E0F11; color: #0E0F11;">View Clinics</a>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadReferralStats() {
        try {
            const response = await fetch('<?php echo rest_url('ptp-training/v1/trainer/referral-stats'); ?>', {
                headers: { 'X-WP-Nonce': '<?php echo wp_create_nonce('wp_rest'); ?>' }
            });
            const data = await response.json();
            
            document.getElementById('refTotalClicks').textContent = data.totals.clicks;
            document.getElementById('refTotalConversions').textContent = data.totals.conversions;
            document.getElementById('refConversionRate').textContent = data.totals.conversion_rate + '%';
            document.getElementById('refTotalCommission').textContent = '$' + data.totals.commission.toFixed(2);
            
            // Render active referrals
            const container = document.getElementById('activeReferrals');
            if (data.referrals.length === 0) {
                container.innerHTML = '<p style="color: var(--ptp-gray-500);">No referral links created yet. Generate one above!</p>';
            } else {
                container.innerHTML = `
                    <table style="width: 100%; font-size: 14px; border-collapse: collapse;">
                        <thead>
                            <tr style="border-bottom: 1px solid var(--ptp-gray-200);">
                                <th style="text-align: left; padding: 8px 0;">Product</th>
                                <th style="text-align: center; padding: 8px;">Clicks</th>
                                <th style="text-align: center; padding: 8px;">Signups</th>
                                <th style="text-align: right; padding: 8px 0;">Earned</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${data.referrals.map(ref => `
                                <tr style="border-bottom: 1px solid var(--ptp-gray-100);">
                                    <td style="padding: 12px 0;">
                                        <strong>${ref.product_name || 'General'}</strong><br/>
                                        <span style="font-size: 12px; color: var(--ptp-gray-500);">Code: ${ref.referral_code}</span>
                                    </td>
                                    <td style="text-align: center; padding: 12px 8px;">${ref.clicks}</td>
                                    <td style="text-align: center; padding: 12px 8px;">${ref.conversions}</td>
                                    <td style="text-align: right; padding: 12px 0; color: var(--ptp-green); font-weight: 600;">$${parseFloat(ref.commission_earned).toFixed(2)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
    ...
    // Generate referral link
    document.getElementById('generateReferralLink')?.addEventListener('click', async function() {
        this.disabled = true;
        this.innerHTML = '<span class="ptp-spinner" style="width:16px;height:16px;border-width:2px;"></span> Generating...';
        
        const type = document.getElementById('referralType').value;
        
        try {
            const response = await fetch('<?php echo rest_url('ptp-training/v1/trainer/referral-link'); ?>', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    'X-WP-Nonce': '<?php echo wp_create_nonce('wp_rest'); ?>' 
                },
                body: JSON.stringify({ type: type, product_id: 0 })
            });
            
            const data = await response.json();
            
            document.getElementById('referralLinkUrl').value = data.url;
        ...
        this.disabled = false;
        this.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg> Generate Link';
    });
    
    // Copy referral link
    document.getElementById('copyReferralLink')?.addEventListener('click', function() {
        const input = document.getElementById('referralLinkUrl');
        input.select();
        document.execCommand('copy');
        showToast('Link copied to clipboard!', 'success');
    });
    
    // Load referral stats when tab is shown
    document.querySelector('[data-tab="referrals"]')?.addEventListener('click', loadReferralStats);
});
</script>
